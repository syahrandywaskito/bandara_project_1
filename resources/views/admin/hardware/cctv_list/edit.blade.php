                ...
                <form class="space-y-4 md:space-y-6" action="{{route('list.cctv.update', $cctv_list->id)}}" method="POST">
                  ...
                  @method('PUT')
                    <div>
                        ...
                        <input type="text" name="name" id="email" class="bg-gray-50 border-2 border-gray-100 text-gray-900 text-xs md:text-sm rounded-lg block w-full p-2.5 focus:ring-indigo-400 focus:ring-2 focus:outline-none" value="{{$cctv_list->name}}">
                    </div>
                    <button type="submit" class="text-xs md:text-sm w-full text-gray-100 font-montserrat bg-gradient-to-r from-indigo-600 to-blue-500 hover:-translate-y-1 hover:scale-105 transition duration-300 focus:ring-4 focus:outline-none font-semibold rounded-lg px-5 py-2.5 text-center shadow-md dark:from-indigo-800 dark:to-indigo-900">
                      Ubah
                    </button>
                </form>
            </div>
